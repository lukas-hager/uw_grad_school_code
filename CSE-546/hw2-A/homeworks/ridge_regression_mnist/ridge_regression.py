...
def predict(x: np.ndarray, w: np.ndarray) -> np.ndarray:
    ...
    """
    output = x @ w
    max_vals = np.argmax(output, axis = 1)
    return(max_vals)
...
def one_hot(y: np.ndarray, num_classes: int) -> np.ndarray:
    ...
    for val in y:
        ...
        array[val] = 1.
        one_hot_arrays.append(array)
    
    y_out = np.vstack(one_hot_arrays)
    return(y_out)
...
def main():
    ...
    print(
        f"\tTrain Error: {np.average(1 - np.equal(y_train_pred, y_train)) * 100:.4g}%"
    )
    print(f"\tTest Error:  {np.average(1 - np.equal(y_test_pred, y_test)) * 100:.4g}%")
    ...
    print('The 95 percent confidence interval is {0:.2f} p/m {1:.4f}'.format(
        np.average(1 - np.equal(y_test_pred, y_test)) * 100,
        conf_int)
    )
...
# predict(
#     np.array([[2,3], [3,4], [10,9], [14,2]]),
#     train(np.array([[2,3], [3,4], [10,9], [14,2]]), one_hot([2, 3, 1, 0], 4), 0)
# )
