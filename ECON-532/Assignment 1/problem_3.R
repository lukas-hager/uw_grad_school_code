...
v_cov <- function(b_val,X_val,Y_val,Z_val,W_val){
  ...
  G <- t(g_i_mat) %*% g_i_mat / n
  omega <- g_i(b_val, X_val, Y_val, Z_val) %*% t(g_i(b_val, X_val, Y_val, Z_val)) / n
...
W_2_inv <- t(Z) %*% (Z * as.numeric(resids_sq)) / dim(X)[1]
W_2 <- solve(W_2_inv)

# second step optimization

optimization_2 <- optim(rep(0,dim(X)[2]), 
                        gmm_crit, 
                        X_val = X, 
                        Y_val = Y, 
                        Z_val = Z,
