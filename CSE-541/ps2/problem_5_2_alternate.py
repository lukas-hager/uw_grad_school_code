...
if exists('/Users/hlukas/git/uw_grad_school_code/CSE-541/ps2/5g_3_dict_a.pkl'):
    with open('/Users/hlukas/git/uw_grad_school_code/CSE-541/ps2/5_3_dict_a.pkl', 'rb') as f:
    ...
    with open('/Users/hlukas/git/uw_grad_school_code/CSE-541/ps2/5_3_dict_a.pkl', 'wb') as f:
...
plt.savefig('/Users/hlukas/Google Drive/Grad School/2021-2022/Winter/CSE 541/Homework/Homework 2/5_2_a.png', dpi = 300)
plt.close()
...
for a in [0., 1.]:
    print(a)
    for n in [int(10. + 2. ** i) for i in np.arange(1.,2.)]:
        print(n)
        np.random.seed(99)
        X_vals = generate_samples(a, n)
        #print(pd.DataFrame(X_vals[:6].T).describe())
        #np.random.seed(99)
        I_t_sim = greedy(X_vals, d, N)
        lambda_hat = (np.array([np.sum(I_t_sim == i) for i in range(n)]) / N).reshape(-1,1)
        print(lambda_hat)
